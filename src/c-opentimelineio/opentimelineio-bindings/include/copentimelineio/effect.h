...
OTIO_API RetainerEffect* Effect_create(const char* name, const char* effect_name, AnyDictionary* metadata);
OTIO_API RetainerEffect* Effect_clone(Effect* self, OTIOErrorStatus* error_status);

OTIO_API bool   Effect_is_equivalent_to(Effect* self, OTIOSerializableObject* other);

OTIO_API char* Effect_schema_name(Effect* self);
OTIO_API int   Effect_schema_version(Effect* self);

OTIO_API char* Effect_effect_name(Effect* self);
OTIO_API void  Effect_set_effect_name(Effect* self, const char* effect_name);
OTIO_API char* Effect_name(Effect* self);
OTIO_API void  Effect_set_name(Effect* self, const char* name);
OTIO_API AnyDictionary* Effect_metadata(Effect* self);

OTIO_API void Effect_string_destroy(char*);
...
OTIO_API bool Effect_to_json_file(
                ...
                int              indent);
OTIO_API char* Effect_to_json_string(
                Effect* self, 
                OTIOErrorStatus* error_status, 
                int indent);
